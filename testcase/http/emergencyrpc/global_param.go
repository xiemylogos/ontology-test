...
func TestGlobalParam(ctx *testframework.TestFrameworkContext) bool {
	...
	fmt.Println(types.AddressFromPubKey(originAdmin.PubKey()), originAdmin.Address)
	// init global params
	...
	newAdmin, err := ctx.GetDefaultAccount()
	if err != nil {
		ctx.LogError("Wallet.NewAccount error:%s", err)
		return false
	}
	fmt.Println(types.AddressFromPubKey(newAdmin.PubKey()), newAdmin.Address)
...
func setParam(account *account.Account, ontSdk *sdk.OntologySdk, params *global_params.Params) {
	bf := new(bytes.Buffer)
	params.Serialize(bf)
	ontSdk.Rpc.InvokeNativeContract(0, 0, account, 0, nutils.ParamContractAddress,
...
func transferAdmin(orignAdmin *account.Account, newAdminAddress common.Address, ontSdk *sdk.OntologySdk) {
	...
	ontSdk.Rpc.InvokeNativeContract(0, 0, orignAdmin, 0, nutils.ParamContractAddress,
...
func accpetAdmin(newAdmin *account.Account, ontSdk *sdk.OntologySdk) {
	...
	adminBuffer := new(bytes.Buffer)
	destinationAdmin.Serialize(adminBuffer)
	ontSdk.Rpc.InvokeNativeContract(0, 0, newAdmin, 0, nutils.ParamContractAddress,
...
func getParam(paramNameList *global_params.ParamNameList, ontSdk *sdk.OntologySdk) (*global_params.Params, error) {
	...
	result, err := ontSdk.Rpc.PrepareInvokeNativeSmartContract(0, nutils.ParamContractAddress,
